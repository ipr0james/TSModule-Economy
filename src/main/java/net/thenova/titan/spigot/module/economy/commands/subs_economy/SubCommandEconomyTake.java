...
@CommandUsage(
        min = 2,
        usage = "economy take <player> <amount>",
        description = "Take money away from a player"
)
public final class SubCommandEconomyTake extends SpigotCommand<SpigotSender> implements CommandPermission<SpigotSender> {

    public SubCommandEconomyTake() {
        super("take");
    }
    ...
    public void execute(SpigotSender sender, CommandContext context) {
        ...
        user.getModule(UserEconomy.class).take(amount);
        if(user.isOnline()) {
            MessageHandler.INSTANCE.build("module.economy.economy.take.complete.user")
        ...
        MessageHandler.INSTANCE.build("module.economy.economy.take.complete.sender")
                .placeholder(new Placeholder("amount", UNumber.displayCurrencyExactly(amount)),
    ...
    public boolean hasPermission(SpigotSender sender) {
        return sender.getSender().hasPermission("titan.command.economy.take");
    }
}
